...
public class ArticlesDAO {
    ...
    public static List<Articles> newsRelated(int artId) {
        Articles art = com.messi.king.messinews.model.dao.ArticlesDAO.findById(artId);
        List<Articles> arts = com.messi.king.messinews.model.dao.ArticlesDAO.findByCatId(art.getCategories_id());
        int pcatId = CategoriesDAO.findById(art.getCategories_id()).getParent_cate_id();


        if (arts.size() < 5) {
            List<Articles> artsByPCat = com.messi.king.messinews.model.dao.ArticlesDAO.findByPCatId(pcatId);
...
//            return articlesList;
    ...
    public static void edit(int id, String title, String abstractContent, String content, int cateId) {
        final String query = "update articles set title= :title, abstract_content= :abstract_content, content= :content, categories_id= :categories_id, status = -1  where id = :id";
        try (Connection con = DbUtils.getConnection()) {
    ...
    public static List<Articles> findByTagId(int tag_id) {
        ...
        try (Connection con = DbUtils.getConnection()) {
            ...
            for (ArticleHasTag articleHasTag : artsId) {
                art = com.messi.king.messinews.model.dao.ArticlesDAO.findById(articleHasTag.getArticle_id());
                if (art.getStatus() == 1) {
                    arts.add(art);
                }
            }
