...
<form id="form" action="" method="post" class="d-flex justify-content-center w-100 h-100 bgColorGray">
    <div class="mt-5 p-4" style="background-color: white; width: 30%">
        <div class="d-flex justify-content-between">
            <h4>
                <a href="${pageContext.request.contextPath}/Account/Login" class="mr-2">
        ...
        <input id="username" name="username" type="text" placeholder="Tài khoản" class="w-100 pl-2 inputStyle mt-1"
               required>
        <input name="rawpwd" id="pwd" type="password" placeholder="Mật khẩu" class="w-100 pl-2 inputStyle mt-3"
               required>
        ...
        <div class="dropdown mt-3">
            <button id="btRole"
                    class="btn inputStyle dropdown-toggle w-100 d-flex justify-content-between align-items-center "
                    type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                Vai trò
            </button>
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <a class="dropdown-item"
                   onclick="chooseRole('${pageContext.request.contextPath}/Account/Register?role=1','Độc giả')"> Độc
                    giả</a>
                <a class="dropdown-item"
        ...
        <c:if test="${email.length()!=0}">
            <input id="email" name="email" type="email" placeholder="Email" class="w-100 pl-2 inputStyle mt-3"
        ...
        <div class="g-recaptcha my-3" data-sitekey="6LfaReUlAAAAANR9cnIhU58_Hwu-bffKvC4mr-6H"></div>
...
<script>
    ...
    $('#form').on('submit', function (e) {
        ...
        $.getJSON('${pageContext.request.contextPath}/Account/IsAvailable?username=' + username + '&email=' + email, (data1) => {
            if (data1 == false) {
                alert('Tên tài khoản hoặc email đã được sử dụng.')
            }
            else{
                $.getJSON('${pageContext.request.contextPath}/Account/CheckCaptcha?captcha=' + captcha, (data2) => {
                    if (data2 == false) {
                        alert('Chưa xác nhận mã Captcha! Vui lòng thử lại')
                    }
                    else {
